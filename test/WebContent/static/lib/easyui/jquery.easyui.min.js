...
//try{		//额外修改代码 ，用于表单提交后出现SESSION失效的情况
	eval('(' + data +')');
	opts.success(data);
/*}
catch (e) {
	alert('当前SESSION失效，请重新登录。');			
...
//_750.apply(this,arguments);
	//alert('当前SESSION失效，请重新登录。');			//此代码是额外添加，处理SESSION失效的情况
	//top.location.href = 'login.html';
}});
...
if (ck === true) {								//绑定月单击事件
	$(_96a).combo("panel").delegate('td.calendar-menu-month', 'click', function() {
		var year = $(_96a).combo("panel").find('input.calendar-menu-year').val(),             
			newMonth = $(this).attr('abbr');
		$(_96a).combo("panel").find('td.calendar-day[abbr="' + year + ',' + newMonth + ',1"]').click();
	});
